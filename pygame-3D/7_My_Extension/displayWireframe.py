...
class ProjectionViewer:
    ...
    def display(self):
        ...
        for wireframe in self.wireframes.values():
            ...
            centerXY = (int(wireframe.findCentre()[0]), int(wireframe.findCentre()[1]))
            
            if self.displayEdges:
                for n1, n2 in wireframe.edges:
                    pygame.draw.aaline(self.screen, self.edgeColour, wireframe.nodes[n1][:2], wireframe.nodes[n2][:2], 1)

            pygame.draw.aaline(self.screen, self.edgeColour, centerXY, (200, 150), 1)

            if self.displayNodes:
                for node in wireframe.nodes:
                    pygame.draw.circle(self.screen, self.nodeColour, (int(node[0]), int(node[1])), self.nodeRadius, 0)

            pygame.draw.circle(self.screen, self.nodeColour, centerXY, int(self.nodeRadius*1.5), 0)
